...
export class GameScene extends Phaser.Scene {
  ...
  init(data: { phaseId: number }) {
    ...
    this.bossHpBar = null;
    this.bossLastSpawn = 0;
    this.convoyObject = null;
    this.convoyRouteIdx = 0;
    this.convoyProgress = 0;
    this.ammoCount = 99;
    this.ammoPickups = [];
    this.sniperTowerObjects = [];
  }
  ...
  create() {
    ...
    if (this.phaseData.patrol) {
      for (let i = 0; i < this.enemies.length; i++) {
        ...
        e.setData("patrol", true);
        e.setData("patrolAngle", (i / this.enemies.length) * Math.PI * 2);
        e.setData("baseX", e.x);
        e.setData("baseY", e.y);
        e.setData("patrolRadius", 60 + (i % 3) * 20);
      }
    }
    ...
    if (this.phaseData.sniperTowers) {
      for (const t of this.phaseData.sniperTowers) {
        ...
        const base = this.add.rectangle(0, 8, TILE-4, TILE/2, 0x554444);
        base.setStrokeStyle(2, 0x332222, 0.5);
        const tower = this.add.rectangle(0, -8, TILE-10, TILE-10, 0x776655);
        tower.setStrokeStyle(2, 0x443322, 0.5);
        const scope = this.add.circle(0, -8, 6, 0xff3333, 0.8);
        ...
        const towerObj = this.add.container(tx, ty, [base, tower, scope, tag]);
        towerObj.setData("health", t.hp);
        towerObj.setData("maxHealth", t.hp);
        towerObj.setData("lastShot", 0);
    ...
    if (this.phaseData.convoy) {
      ...
      const body = this.add.rectangle(0, 0, TILE-8, TILE-12, 0x4a4a8a);
      body.setStrokeStyle(2, 0x2a2a5a, 0.5);
      const turret = this.add.rectangle(0, -8, 12, 6, 0x6a6aaa);
      const tag = this.add.text(0, -22, "JIPE", { fontSize:"8px", color:"#aabbff", fontFamily:"monospace", fontStyle:"bold" }).setOrigin(0.5);
      this.convoyObject = this.add.container(sx, sy, [body, turret, tag]);
      this.convoyRouteIdx = 0;
      this.convoyProgress = 0;
    }
    ...
    if (this.phaseData.boss) {
      ...
      hull.setStrokeStyle(3, 0x111111, 0.6);
      const turret = this.add.rectangle(0, 0, TILE-10, 10, 0x555555);
      const gun = this.add.rectangle(20, 0, 20, 6, 0x333333);
      const tag = this.add.text(0, -32, "GENERAL GORILA", { fontSize:"10px", color:"#ff4444", fontFamily:"monospace", fontStyle:"bold" }).setOrigin(0.5);
      this.bossObject = this.add.container(bx, by, [hull, turret, gun, tag]);
      this.bossObject.setData("health", this.phaseData.boss.hp);
      this.bossObject.setData("maxHealth", this.phaseData.boss.hp);
      this.bossObject.setData("type", "boss");
      this.bossHpBar = this.add.rectangle(bx, by - 40, 60, 6, 0xff0000).setDepth(51);
      this.bossHpBar.setStrokeStyle(1, 0x000000, 0.5);
    }
  ...
  update() {
    ...
    for (const e of this.enemies) {
      ...
      if (e.getData("patrol")) {
        ...
        const radius = e.getData("patrolRadius") as number;
        e.x = baseX + Math.cos(angle) * radius;
        e.y = baseY + Math.sin(angle) * radius;
      }
    ...
    if (this.bossObject && this.bossObject.active && this.phaseData.boss) {
      ...
      const bossMaxHp = boss.getData("maxHealth") as number;
      ...
      if (this.time.now - this.bossLastSpawn > this.phaseData.boss.spawnInterval) {
        ...
        for (let i = 0; i < 2; i++) {
          const sx = boss.x + (Math.random() - 0.5) * 80;
          const sy = boss.y + (Math.random() - 0.5) * 80;
          const enemy = this.createEnemy(sx, sy, "soldier");
          this.enemies.push(enemy);
          const eBar = this.add.rectangle(sx, sy - 28, 24, 3, 0xff3333).setDepth(51);
          eBar.setData("enemyIndex", this.enemies.length - 1);
          this.enemyHealthBars.push(eBar);
      ...
      if (bossHp <= 0) {
        ...
        audio.explosion();
        for (let i = 0; i < 20; i++) {
          const p = this.add.circle(px + (Math.random()-0.5)*60, py + (Math.random()-0.5)*60, 4+Math.random()*8, 0xff4400);
          this.tweens.add({ targets: p, alpha: 0, scaleX: 3, scaleY: 3, duration: 600, onComplete: () => p.destroy() });
        }
      }
    }
    ...
    for (const tower of this.sniperTowerObjects) {
      ...
      for (const s of this.squad) {
        if (!s.active || !s.getData("alive")) continue;
        const d = Phaser.Math.Distance.Between(tower.x, tower.y, s.x, s.y);
        if (d < minDist) { minDist = d; target = s; }
      }
      if (target && minDist < TILE * 15) {
        tower.setData("lastShot", this.time.now);
        const a = Math.atan2(target.y - tower.y, target.x - tower.x);
        const b = this.add.rectangle(tower.x, tower.y - 8, 10, 3, 0xcc0000);
        b.setRotation(a);
        audio.shoot();
        this.tweens.add({
          targets: b, x: tower.x + Math.cos(a) * 700, y: (tower.y - 8) + Math.sin(a) * 700, duration: 250,
          onUpdate: () => {
            for (const s of this.squad) {
              if (!s.active || !s.getData("alive")) continue;
              if (Phaser.Math.Distance.Between(b.x, b.y, s.x, s.y) < TILE / 2) { this.damageSoldier(s, 2); b.destroy(); return; }
            }
          },
    ...
    if (this.convoyObject && this.convoyObject.active && this.phaseData.convoy) {
      ...
      const current = route[this.convoyRouteIdx];
      const next = route[(this.convoyRouteIdx + 1) % route.length];
      ...
      if (this.convoyProgress >= 1) {
        this.convoyProgress = 0;
        this.convoyRouteIdx = (this.convoyRouteIdx + 1) % route.length;
      } else {
        this.convoyObject.x = cx + (nx - cx) * this.convoyProgress;
        this.convoyObject.y = cy + (ny - cy) * this.convoyProgress;
      }
    }
    ...
    for (const ammo of this.ammoPickups) {
      ...
      for (const s of this.squad) {
        if (!s.active || !s.getData("alive")) continue;
        if (Phaser.Math.Distance.Between(s.x, s.y, ammo.x, ammo.y) < TILE * 0.5) {
          this.ammoCount += ammo.getData("ammo") as number;
          audio.hit();
          ammo.destroy();
          break;
        }
      ...
      onUpdate: () => {
        ...
        for (const tower of this.sniperTowerObjects) {
          ...
          if (Phaser.Math.Distance.Between(b.x, b.y, tower.x, tower.y) < TILE * 0.6) {
            let hp = (tower.getData("health") as number) - 1;
            tower.setData("health", hp);
            audio.hit();
            ...
            if (hp <= 0) {
              ...
              audio.explosion();
              for (let i = 0; i < 12; i++) {
                const p = this.add.circle(px + (Math.random()-0.5)*40, py + (Math.random()-0.5)*40, 3+Math.random()*6, 0xff4400);
                this.tweens.add({ targets: p, alpha: 0, scaleX: 3, scaleY: 3, duration: 500, onComplete: () => p.destroy() });
              }
            }
            b.destroy(); return;
  ...
  hitBoss() {
    ...
    let hp = (this.bossObject.getData("health") as number) - 1;
    this.bossObject.setData("health", hp);
    audio.hit();
    const hull = this.bossObject.getAt(0) as Phaser.GameObjects.Shape;
    if (hull && hull.setTint) {
      hull.setTint(0xffffff);
      this.time.delayedCall(100, () => { if (this.bossObject && this.bossObject.active && hull.active) hull.clearTint(); });
    }
  }
    ...
    if (hp <= 0) {
      ...
      if (this.phaseData.lowAmmo) {
        const ammoBox = this.add.container(px, py, [
          ...
          this.add.text(0, -2, "A", { fontSize: "8px", color: "#fff", fontFamily: "monospace", fontStyle: "bold" }).setOrigin(0.5),
        ]);
        ammoBox.setData("ammo", 5);
        this.ammoPickups.push(ammoBox);
      }
    } else {
      e.setData("health", hp);
